...
import type { DemoSample } from "@/components/DemoDatasetPicker";
...
function crmContextKey(
  ...
  if (company.domain) {
    return `${company.domain.toLowerCase()}::${name}`;
  }

  if (company.website) {
    return `${company.website.toLowerCase()}::${name}`;
  }
  ...
  useEffect(() => {
    ...
    async function loadSample() {
      ...
      try {
        ...
        setError(
          "Could not load this sample dataset."
        );
      }
    }
    ...
    // initialSample is intentionally the trigger.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
      ...
      <div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
        ...
        <div className="border-t border-slate-100 bg-emerald-50/50 px-6 py-3 text-xs leading-5 text-emerald-800">
          Privacy: this analysis is temporary and is not
          added to shared Supabase history.
        </div>
