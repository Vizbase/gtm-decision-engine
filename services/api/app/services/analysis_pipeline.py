...
class AnalysisPipeline:
    ...
    async def run(
        ...
        enrichment_by_company = await self._enrich_companies(
            normalized_companies,
            use_website_enrichment,
        )

        crm_contexts = self.crm_provider.get_contexts(
            normalized_companies
        )
        ...
        for rank, scored in enumerate(
            ...
            enrichment = enrichment_by_company.get(
                ...
                WebsiteEnrichment(
                    reachable=False,
                    error="No enrichment result available.",
                ),
            )
    ...
    async def _enrich_companies(
        ...
        enabled: bool,
    ) -> dict[str, WebsiteEnrichment]:
        if not enabled:
            return {
                company_key(company): WebsiteEnrichment(
                    reachable=False,
                    error="Website enrichment disabled.",
                )
                for company in companies
            }

        tasks = [
            self.enrichment_provider.enrich(company)
            for company in companies
        ]
        ...
        return {
            company_key(company): enrichment
            for company, enrichment in zip(
                companies,
                enrichment_results,
            )
        }
